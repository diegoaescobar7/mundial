...
if ($app->cargarVista("detallesajax") === false) {
    $app->error("No se encontro la vista. ajax.");
}
$vista = new VistaDetallesAjax();
...
while ($registro = $modelo->getRegistro()) {
    ...
    $total = $total + $vlrtra;
    agregaCelda($arreglo->$nit->$nrofac->nrofac, "valor:$nrofac;clase:$styloc;titulo:Numero Factura");
  ...
  $fectra = date("m-d-Y", strtotime($fectra));
    ...
    agregaCelda($arreglo->$nit->$nrofac->fectra, "valor:$fectra;clase:$styloc;titulo:Fecha Factura");
    
    agregaCelda($arreglo->$nit->$nrofac->fecven, "valor:$fecven;clase:$styloc;titulo:Fecha Vencimiento");
    agregaCelda($arreglo->$nit->$nrofac->vlrtra, "valor:$vlrtra;clase:$stylo;titulo:Total con Iva");
    if($base == 0 or $base == "" or $base == null)
    {
       agregaCelda($arreglo->$nit->$nrofac->base, "valor:--;clase:$stylo;titulo:Total sin Iva"); 
       agregaCelda($arreglo->$nit->$nrofac->iva, "valor:--;clase:$stylo;titulo:Iva");
    }
    else{
        ...
        agregaCelda($arreglo->$nit->$nrofac->base, "valor:$sinivaFull;clase:$stylo;titulo:Total sin Iva");
        agregaCelda($arreglo->$nit->$nrofac->iva, "valor:$iva2 %;clase:$stylo;titulo:Iva");
        
    }
    
    if($descuento == 0 or $descuento == "" or $descuento == null){
        agregaCelda($arreglo->$nit->$nrofac->descuento, "valor:--;clase:$stylo;titulo:Descuentos");
    }
    else{
        $descuento = number_format($descuento, 1);
        agregaCelda($arreglo->$nit->$nrofac->descuento, "valor:$descuento %;clase:$stylo;titulo:Descuentos");
    }
